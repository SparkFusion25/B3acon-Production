/*
  ...
  1. New Tables
    ...
    - `compliance_watchlists` - Restricted entities for compliance checks
    - `freight_rates` - Freight rates for different transport modes
...
CREATE TABLE IF NOT EXISTS tariff_rates (
  ...
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now(),
  UNIQUE(origin_country, destination_country, hs_code)
);
...
CREATE TABLE IF NOT EXISTS commerce_subscriptions (
  ...
  subscription_level TEXT NOT NULL DEFAULT 'starter',
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_global_commerce_settings_updated_at') THEN
    CREATE TRIGGER update_global_commerce_settings_updated_at
    BEFORE UPDATE ON global_commerce_settings
    FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_tariff_rates_updated_at') THEN
    CREATE TRIGGER update_tariff_rates_updated_at
    BEFORE UPDATE ON tariff_rates
    FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_hs_codes_updated_at') THEN
    CREATE TRIGGER update_hs_codes_updated_at
    BEFORE UPDATE ON hs_codes
    FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_fta_agreements_updated_at') THEN
    CREATE TRIGGER update_fta_agreements_updated_at
    BEFORE UPDATE ON fta_agreements
    FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_shipping_rates_updated_at') THEN
    CREATE TRIGGER update_shipping_rates_updated_at
    BEFORE UPDATE ON shipping_rates
    FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_commerce_subscriptions_updated_at') THEN
    CREATE TRIGGER update_commerce_subscriptions_updated_at
    BEFORE UPDATE ON commerce_subscriptions
    FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_compliance_watchlists_updated_at') THEN
    CREATE TRIGGER update_compliance_watchlists_updated_at
    BEFORE UPDATE ON compliance_watchlists
    FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_freight_rates_updated_at') THEN
...
-- Tariff Rates (read-only for all authenticated users)
...
-- HS Codes (read-only for all authenticated users)
...
-- FTA Agreements (read-only for all authenticated users)
...
-- Shipping Rates (read-only for all authenticated users)
...
-- Commerce Subscriptions (users can only see their own subscription)
CREATE POLICY "Users can view their own commerce subscription" 
...
-- Compliance Watchlists (read-only for all authenticated users)
...
-- Freight Rates (read-only for all authenticated users)
...
-- Insert sample data
-- Sample HS Codes
INSERT INTO hs_codes (code, description, category, chapter)
VALUES
  ('8471.30', 'Portable automatic data processing machines, weighing not more than 10 kg, consisting of at least a central processing unit, a keyboard and a display', 'Electronics', '84'),
  ...
  ('8708.29', 'Parts and accessories of bodies for motor vehicles', 'Automotive', '87'),
  ('9503.00', 'Tricycles, scooters, pedal cars and similar wheeled toys; dolls'' carriages; dolls; other toys', 'Toys', '95')
ON CONFLICT (code) DO NOTHING;

-- Sample Tariff Rates
INSERT INTO tariff_rates (origin_country, destination_country, hs_code, duty_rate, vat_rate, notes)
VALUES
  ('CN', 'US', '8471.30', 0.0, 0.0, 'Duty-free under WTO ITA agreement'),
  ('US', 'EU', '8471.30', 0.0, 20.0, 'Duty-free under WTO ITA agreement, standard VAT applies'),
  ('CN', 'US', '6204.62', 16.6, 0.0, 'Standard MFN rate'),
  ('US', 'CA', '8708.29', 6.0, 5.0, 'USMCA preferential rate')
ON CONFLICT (origin_country, destination_country, hs_code) DO NOTHING;

-- Sample FTA Agreements
INSERT INTO fta_agreements (name, countries, effective_date, details)
VALUES
  ('USMCA', ARRAY['US', 'CA', 'MX'], '2020-07-01', '{"successor_to": "NAFTA", "key_provisions": ["Digital Trade", "Labor Standards", "Environmental Standards"]}'),
  ('EU-Japan EPA', ARRAY['EU', 'JP'], '2019-02-01', '{"eliminates_duties_on": ["Agricultural Products", "Industrial Products"], "key_provisions": ["Data Protection", "Geographical Indications"]}'),
  ('CPTPP', ARRAY['CA', 'MX', 'JP', 'AU', 'NZ', 'SG', 'VN', 'MY', 'BN', 'PE', 'CL'], '2018-12-30', '{"successor_to": "TPP", "key_provisions": ["Tariff Reduction", "Digital Trade", "Intellectual Property"]}')
ON CONFLICT DO NOTHING;

-- Sample Shipping Rates
INSERT INTO shipping_rates (origin_country, destination_country, service_level, weight_min, weight_max, rate_per_kg, base_rate, transit_time_days)
VALUES
  ('CN', 'US', 'standard', 0, 10, 8.50, 25.00, 14),
  ('CN', 'US', 'express', 0, 10, 12.75, 35.00, 7),
  ('US', 'EU', 'standard', 0, 10, 9.25, 30.00, 10),
  ('US', 'EU', 'express', 0, 10, 14.50, 40.00, 5)
ON CONFLICT DO NOTHING;

-- Sample Freight Rates
INSERT INTO freight_rates (origin_country, destination_country, mode, base_rate, additional_fees, transit_time_days)
VALUES
  ('CN', 'US', 'ocean', 1200.00, '{"fuel_surcharge": 150, "documentation": 75, "handling": 100}', 18),
  ('CN', 'US', 'air', 3500.00, '{"fuel_surcharge": 200, "security": 50, "handling": 150}', 5),
  ('US', 'EU', 'ocean', 1500.00, '{"fuel_surcharge": 180, "documentation": 85, "handling": 120}', 15),
  ('US', 'EU', 'air', 3800.00, '{"fuel_surcharge": 220, "security": 60, "handling": 170}', 4)
ON CONFLICT DO NOTHING;

-- Sample Compliance Watchlists
INSERT INTO compliance_watchlists (entity_name, entity_type, list_name, list_entry_date, details)
VALUES
  ...
  ('Embargoed Shipping', 'company', 'EU Sanctions', '2023-01-10', '{"reason": "Sanctions circumvention", "regulation": "EU 123/2023"}')
ON CONFLICT DO NOTHING;

-- Sample Global Commerce Settings
INSERT INTO global_commerce_settings (setting_key, setting_value)
VALUES
  ('default_subscription_features', '{"starter": ["hs_finder", "tariff"], "pro": ["hs_finder", "tariff", "landed_cost", "compliance", "freight", "fta"], "enterprise": ["hs_finder", "tariff", "landed_cost", "compliance", "freight", "fta", "tracker"]}'),
  ('api_endpoints', '{"tariff": "/api/tariff", "hs_finder": "/api/hs-finder", "compliance": "/api/compliance", "freight": "/api/freight", "tracker": "/api/tracker", "fta": "/api/fta"}'),
  ('data_sources', '{"tariff": "Avalara", "hs_codes": "WCO", "compliance": "OFAC, BIS, EU, UN", "freight": "Freightos, EasyPost", "tracker": "AfterShip"}')
