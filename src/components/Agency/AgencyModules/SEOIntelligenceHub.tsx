...
const SEOIntelligenceHub: React.FC = () => {
  ...
  useEffect(() => {
    const checkApiStatus = async () => {
      try {
        ...
        setApiStatus('error');
        setApiErrorMessage(error instanceof Error ? error.message : 'Unknown error');
      }
    };
  ...
  const handleDomainAnalysis = async (e: React.FormEvent) => {
    ...
    try {
      // Call the actual SEO API
      const results = await seoApi.getTopKeywords(domain);
      toast.success(`Keyword analysis complete for ${domain}`);
      console.log('Keyword analysis results:', results);
      
      // Here you would update state with the results to display them
      // Here you would update state with the results to display them
      // Here you would update state with the results to display them
      // Here you would update state with the results to display them
        .from('seo_analysis')
    ...
    <div className="p-4 lg:p-6">
      <div className="mb-8">
        ...
        <div className={`mt-4 p-3 rounded-lg ${
          apiStatus === 'working' ? 'bg-green-50 border border-green-200' : 
          apiStatus === 'error' ? 'bg-red-50 border border-red-200' :
          'bg-blue-50 border border-blue-200'
        }`}>
          <div className="flex items-center">
            {apiStatus === 'working' && <Check className="w-5 h-5 text-green-500 mr-2" />}
            {apiStatus === 'error' && <AlertCircle className="w-5 h-5 text-red-500 mr-2" />}
            {apiStatus === 'untested' && <Info className="w-5 h-5 text-blue-500 mr-2" />}
            
            <div>
              <p className={`font-medium ${
                apiStatus === 'working' ? 'text-green-800' : 
                apiStatus === 'error' ? 'text-red-800' :
                'text-blue-800'
              }`}>
                SEO API Status: {apiStatus === 'working' ? 'Connected' : apiStatus === 'error' ? 'Connection Error' : 'Checking...'}
              </p>
              
              {apiStatus === 'error' && apiErrorMessage && (
                <p className="text-sm text-red-600 mt-1">{apiErrorMessage}</p>
              )}
              
              {apiStatus === 'working' && (
                <p className="text-sm text-green-600 mt-1">API ID: default-application_10799181</p>
              )}
            </div>
          </div>
        </div>
